...
```{r}
bulkHgncNc <- filterBulkGenes(bulkData = gtexMuscle, hgncData = completeHGNC, ncData = ncRNA)

saveRDS(bulkHgncNc, file = "data/processed_data/bulk_rna_filter_ncRNA_HGNC/bGTExMuscle.rds")
```
...
```{r}
bulkHgncNc1 <- filterBulkGenes(bulkData = aeCcfBulk1, hgncData = completeHGNC, ncData = ncRNA) %>%
...
| AtheroExpress ccfRNA         | `cpm > 1`      | 15 (batch 1: 150 patients, batch 2: 248 patients) | 8000                                                       |
...
lapply(list(cpm1, cpm2), function(c) {
...
for(i in seq_along(dgeBatches)) {
  ...
  toKeepGenes <- rowSums(cpmBatches[[i]] > 1) >= 15
  # Patients need to have at least x genes expressed
  toKeepPatients <- colSums(cpmBatches[[i]] > 1) >= 8000
...
```{r}
saveRDS(bulkNorm1, "data/preprocessed_data/bAEccfRNA1.rds")
