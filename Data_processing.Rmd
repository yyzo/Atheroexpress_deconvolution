...
rm(scData, scFilter, bulkData, bulkFilter)
gc()
```
...
```{r}
scadenBulk <- bulkFinal %>%
  as.data.frame() %>%
  column_to_rownames("symbol") %>%
  subset(select = -ensembl_gene_id)

write.table(scadenBulk,
            file = "output/scaden_format_files/scaden_bulk_data.txt",
            sep = "\t")

scadenCellTypes <- as.data.frame(Idents(scFinal)) %>%
  rename(Celltype = `Idents(scFinal)`)
  
scadenCounts <- as.data.frame(scFinal[["RNA"]]$counts) %>%
  t()

#Check if the rownames of the two data frames are in the same order.
table(rownames(scadenCounts) == rownames(scadenCellTypes))

write.table(scadenCellTypes,
            file = "output/scaden_format_files/scaden_celltypes.txt",
            sep = "\t", row.names = FALSE)

#rownames(scadenCounts) <- seq_len(nrow(scadenCounts)) - 1

write.table(scadenCounts,
            file = "output/scaden_format_files/scadenOne_counts.txt",
            sep = "\t", row.names = FALSE)
```
