...
do
  scaden simulate -n 10 --pattern "*_counts.txt" --data $dirPathFiles --out $dirPathSimul
    ...
    scaden process "$dirPathSimul/scaden.h5ad" "$dirPathFiles/scaden_bulk_data.txt" --processed_path "$dirPathSimul/processed.h5ad"

  scaden train "$dirPathSimul/processed.h5ad" --steps 10 --model_dir $dirPathModel

  scaden predict --model_dir $dirPathModel "$dirPathFiles/scaden_bulk_data.txt" --outname "predictions_${typeDeconv}_b1500_c7_$i"

  mv "predictions_${typeDeconv}_b1500_c7_$i" $dirPathPred
  
  echo "Moved predictions_${typeDeconv}_b1500_c7_$i to $dirPathPred"
...
for(i in nSimulations){
  path <- paste0("output/", typeDeconv, "/predictions/predictions", typeDeconv, "_b1500_c7_", i)
  file <- paste0("pred", i)
...
#                 "output/export_files/AE_scaden_deconvolution_plaque_7clusters_avgprop.csv")
```
