...
```{r}
typeDeconv = "scaden_plaque"
nBulk = "1000"
nSimulations = seq(30)

for(i in nSimulations){
  path <- paste0("output/", typeDeconv, "/predictions/predictions_", typeDeconv, "_b", nBulk, "_c7_", i)
  file <- paste0("pred", i)
...
#                 "output/export_files/AE_scaden_deconvolution_plaque_7clusters_avgprop_bk1000.csv")
...
```{r}
#All predictions
pdf(paste0("output/", typeDeconv, "/predictions/", typeDeconv, "_predictions_graphs_b", nBulk, ".pdf"))
...
#Average prediction
tiff(paste0("output/figures/", typeDeconv, "_average_b", nBulk,".tiff"),
     width = 720,
     height = 720,
     units = "px")
...
pAvg <- ggplot(avgProp, aes(x = Celltype, y = `Average proportion`)) +
  ...
  labs(title = paste(typeDeconv, "predictions"),
       subtitle = paste("Size training set:", nBulk)) +
  scale_y_continuous(name = "Average cell type proportion",
                           limits = c(0, 0.8)) +
  ownMinimalTheme()
...
```{r}
tiff(paste0("output/figures/", typeDeconv, "_sympt2g_b", nBulk,".tiff"),
     width = 720,
     height = 720,
     units = "px")
...
pAvg2g <- ggplot(avgPropSymp, aes(x = Celltype, y = `Average proportion`, fill = Symptoms.2g)) +
  geom_boxplot() +
  labs(title = paste(typeDeconv, "predictions"),
       subtitle = paste("Mild versus severe symptoms/n
                        Size training set:", nBulk),
       fill = "Group") +
...
```{r}
tiff(paste0("output/figures/", typeDeconv, "_sympt4g_b", nBulk,".tiff"),
     width = 720,
     height = 720,
     units = "px")
...
pAvg4g <- ggplot(avgPropSymp, aes(x = Celltype, y = `Average proportion`, fill = Symptoms.4g)) +
  geom_boxplot() +
  labs(title = paste(typeDeconv, "predictions"),
       subtitle = paste("Asymptomatic versus ocular versus TIA versus stroke/n
                        Size training set:", nBulk),
       fill = "Group") +
...
#All patients
compare <- avgPropSymp[1:2]
compare["ownProp"] <- avgPropSymp$`Average proportion`
compare["gemmaProp"] <- filterGemmaPropSymp$`Average proportion`

tiff("output/figures/cor_plaque_all.tiff",
     width = 720,
     height = 720,
     units = "px")
...
ggplot(compare, aes(x = ownProp, y = gemmaProp)) +
  ...
  labs(title = "Correlation average cellular proportions of all patients")
