...
```{r}
fileBasePattern <- "rTS_bAEccfRNACombine_c17_v" # MODIFY HERE
...
```{r}
bulkGenesToCor <- read.table(file = "data/processed_data/scaden_bulk_rna_files/rTS_bAEccfRNACombine_bulk_data.txt", check.names = FALSE) # MODIFY HERE; bulk data used to obtain the predictions from Section 4.3
...
ggsave(paste0("output/plots/boxplot_cell_type_proportion/tiff/scaden_", figPattern, "_predProp.tiff"),
     width = 9, height = 12, units = "cm", dpi = 300) # 7x12 for 10 cell types; 13x13 for 17
```
...
#### 4.5.1.3.4 Different types

```{r}
longPredrAE <- longPredrAE %>%
  mutate(ref = "AE")

longPredrTS <- longPredrTS %>%
  mutate(ref = "TS")

combine <- rbind(longPredrAE, longPredrTS)

pMethods <- ggplot(combine, aes(x = reorder(cellType, avgProportion), y = avgProportion)) +
  geom_boxplot(aes(fill = ref), position = position_dodge(width = 0.8), width = 0.5, linewidth = 0.3, outlier.size = 1, outlier.shape = 1) +
  labs(x = "",
       y = "Estimated cell type proportion") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),
                     breaks = seq(0, 1, by = 0.1)) +
...
```{r}
tiff("output/plots/correlation_heatmap_predictions/scaden_rTS_bAEccfRNACombine_com4_c17.tiff",
