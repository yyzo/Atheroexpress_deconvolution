...
source("functions/functions_figures_R.R")
```
...
```{r}
fileBasePattern <- "rTS_bAEccfRNA1_c17_v" # MODIFY HERE
...
```{r}
predToCorrelateLong <- readRDS(file = "output/export_files/scaden_long_avg_prop_rAEplaque_bAEplaque1_c7_v1.rds") # MODIFY HERE

toCorrelateCol <- "avgProportion" # MODIFY HERE

predToCorrelateWide <- readRDS(file = "output/export_files/scaden_wide_avg_prop_rAEplaque_bAEplaque1_c7_v1.rds") # MODIFY HERE
```

### 4.4.1.1 Dataframe for scatter plots

```{r}
corPredScatter <- predToCorrelateLong %>%
  left_join(widePred, by = "patient") %>%
  rename_with(.fn = ~ str_replace_all(., c(" " = "_", "/" = "_")),
              .cols = -c(patient, cellType, !!sym(toCorrelateCol))) %>%
...
```{r}
widePredHeatmap <- column_to_rownames(widePred, "patient")
predToCorrelateWideHeatmap <- column_to_rownames(predToCorrelateWide, "patient")

corPredHeatmap <- cor(widePredHeatmap, predToCorrelateWideHeatmap) # NOTE: rows = widePredHeatmap; columns = predToCorrelateWideHeatmap
```

## 4.4.2 Correlation of predicted cellular proportions with cell type gene marker

```{r}
bulkGenesToCorrelate <- read.table(file = "data/processed_data/scaden_bulk_rna_files/rTS_bAEccfRNA1_bulk_data.txt") # MODIFY HERE; bulk data used to obtain the predictions from Section 4.3
...
missingGenes <- unlist(geneMarkers)[!unlist(geneMarkers) %in% rownames(bulkGenesToCorrelate)]
```

```{r}
# Load bulk RNA-seq data corresponding to the one used for "widePred"
bulkGeneMarkers <- bulkGenesToCorrelate %>%
...
normBefore <- str_detect(fileBasePattern, "AEccfRNA")
...
if(!normBefore) {
  ...
  print(table(rownames(widePredHeatmap) == rownames(bulkGeneMarkersNorm)))
  
  corGeneMarkers <- cor(widePredHeatmap, bulkGeneMarkersNorm)
} else {
  cat("Bulk has been normalized prior.")
  bulkGeneMarkersT <- as.data.frame(t(bulkGeneMarkers))
  
  print(table(rownames(widePredHeatmap) == rownames(bulkGeneMarkersT)))
  
  corGeneMarkers <- cor(widePredHeatmap, bulkGeneMarkersT)
}
```
...
if(nCellTypes == 7) {
  pOnePred <- pOnePred + coord_fixed(ratio = 10)
} else {
  pOnePred <- pOnePred + coord_fixed(ratio = 30)
}

print(pOnePred)

ggsave(paste0("output/plots/boxplot_cell_type_proportion/tiff/scaden_", figPattern, "_predProp.tiff"),
     width = setWidth, height = setHeight, units = "mm", dpi = 300)
...
```{r}
path <- paste0("output/plots/correlation_scatter_plot/bayesprism_scaden_", figPattern)
...
for(i in (4:ncol(corPredScatter))) {
  tiff(paste0(path, "/bayesprism_scaden_", figPattern, "_", i, "_scatter_cor.tiff"), width = 8, height = 8, units = "in", res = 300)
  ...
  pScatter <- ggplot(corPredScatter, aes(x = .data[[colName]], y = proportionToCor)) +
    ...
    labs(x = paste0("bayesprism_", colName),
         y = "scaden") + # MODIFY HERE; x corresponds to imported data in Section 4.3; y corresponds to imported data in Section 4.4.1
    theme_classic(base_family = "Helvetica") +
...
heatmapPred <- Heatmap(corPredHeatmap, col = heatmapPal, rect_gp = gpar(col = "white", lwd = 1),
        row_title = "Tabula Sapiens reference", column_title = "AtheroExpress reference",
        ...
        layer_fun = function(j, i, x, y, width, height, fill) {
          grid.text(sprintf("%.2f", pindex(corPredHeatmap, i, j)), x, y, gp = gpar(fontsize = 6))
        }
)
```

```{r}
tiff("output/plots/correlation_heatmap_predictions/rAEplaque+TS_bAEplaque1_c7+10_v1.tiff",
...
### 4.5.3.3 Heatmap: between predicted cellular proportions and cell type gene marker
