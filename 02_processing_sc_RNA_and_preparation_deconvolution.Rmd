...
```{r}
bulkToMatch <- read_rds("data/preprocessed_data/bAEccfRNACombine.rds") # MODIFY HERE
...
if(aeRefToUse) {
  ...
  metadata <- scTSClusterFilterGenes131@meta.data # MODIFY HERE
...
table(rownames(scTSCounts131) == cellProfile$cell.ID)
```
...
if(aeRefToUse) {
  ...
  countsToExport <- "scTSCounts131" # MODIFY HERE
}
...
```{r}
meta <- scAEplaqueFilterFreq@meta.data
Idents(scTSCluster101) <- scTSCluster101@meta.data$cell_ontology_class

tiff("TS_cluster_10_presentation.tiff", width = 12, height = 8, units = "in", res = 300)

DimPlot_scCustom(scTSCluster101,
                 reduction = "umap",
                 label.box = TRUE,
                 label.size = 4,
                 repel = TRUE)
...
tiff("TS_cluster_7_presentation.tiff", width = 10, height = 8, units = "in", res = 300)

UMAPPlot(scTSCluster101) + NoLegend()
...
tissueData <- scTSClusterFilterGenes101@meta.data %>%
  select(tissue_in_publication, cluster10) %>%
  rownames_to_column("cellID") %>%
  select(-cellID) %>%
  make_long(cluster10, tissue_in_publication)

p <- ggplot(tissueData, aes(x = x, next_x = next_x, node = node, next_node = next_node, fill = factor(node), label = node)) +
  geom_sankey(flow.alpha = 0.5) +
  geom_sankey_label(color = "black") +
  labs(x = "") +
  theme_sankey() +
  theme(legend.position = "none") +
  scale_fill_manual(values = sample(as.vector(paletteer::paletteer_d("colorBlindness::SteppedSequential5Steps"))))

# svg("plots_thesis/rTS_bAEplaque1_c10_sankey.svg", width = 8, height = 8)
