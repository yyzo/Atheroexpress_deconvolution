...
Below, subtypes of epithelial cells can be grouped together into one major cell type "Epithelial cell". The average predicted cellular proportion equals the sum of all these subtypes.
...
tiff("output/plots/average_predictions_plots/rTS_bAEblood_c17_v2_tissue.tiff", 
     width = 12, 
     height = 8, 
     units = "in", 
     res = 300)

ggplot(avgProportions, aes(fill=tissue, x=reorder(Celltype, `Average proportion`), y=`Average proportion`)) + 
  geom_boxplot(position=position_dodge(width=0.8), width=0.5, alpha=0.5, outlier.shape = NA) +
  scale_fill_viridis(discrete=TRUE, name="Tissue") +
  scale_y_percent() +
  labs(title = "Average predicted cellular proportions: circulating RNA",
       subtitle = "Reference: Tabula Sapiens") +
  ylab("Relative average proportion") +
  xlab("Celltype") +
  theme_ipsum_rc(grid="Y") +
