...
patientData$thrombus.bin <- factor(patientData$thrombus.bin, levels = c(0, 1, -888), labels = c("no/minor", "moderate/heavy", "not judgeable"))
...
patientData$Gender <- factor(patientData$Gender, levels = c(0, 1), labels = c("female", "male"))
```
...
if(str_detect(pattern, "AEccfRNACombine")) {
  ...
  predSympWide$dateok.bin <- factor(predSympWide$dateok.bin, levels = c(0, 1), labels = c("before/on 2009-01-22", "after 2009-01-22"))
}
```
...
```{r}
sum(is.na(predSympWide$CAD_history))

contCol <- c("BMI", "GFR_MDRD")
binCol <- c("SmokerCurrent", "Hypertension.composite", "DM.composite", "PAOD", "CAD_history")

# Impute missing values in columns with continuous data
...
groupColToImpute <- predSympWide %>%
  ...
  mutate(across(all_of(binCol), as.factor))

toImpute <- left_join(contColToImpute, groupColToImpute, by = "patient")

colImputedList <- mice(toImpute, method = "pmm", seed = 2468)
...
sum(is.na(predSympWideImputed$SmokerCurrent))
```
...
covariates <- c("batch", "Age", "BMI", "Gender", "GFR_MDRD", "hsCRP.bin") # MODIFY HERE; for interaction terms, add together in one string ("var1 * var2")
...
# Fit GLM
resGLM <- lapply(formulasGLM, function(f) glm(f, family = glmFamily, data = predSympWideImputed))
...
covariates <- c("batch", "Gender", "Age", "BMI", "CAD_history") # MODIFY HERE
